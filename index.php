...
if($_SESSION['token']){
	...
	if ($user->error) {
		if ($user->error->type== "OAuthException") {
			session_destroy();
			header('Location: /?i=Your token Has Expired.');
		}
	}
}

// bam submit de lay token
if(isset($_POST['submit'])) {
	...
	if ($pos == true) {
		...
		session_destroy();
		header('Location: /?i=Please Generate token Using HTC Apps!');}
}else{} //removed condition
	if(isset($_POST['logout'])) {
		session_destroy();
		header('Location: /?i=Your token Has Deleted.');
	}
	// Catch thong tin chuyen qua param = i va hien thi
	if(isset($_GET['i'])){
